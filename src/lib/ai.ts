...
/**
 * Google Gemini Interactions API (v1beta).
...
async function gemini(
  ...
  // For single message, use plain string (official REST format)
  // For multi-turn, concatenate messages into a single string with role labels
  let input: string;
  ...
  // Interactions API: text is in steps[].content[].text
...
export async function summarizeNote(content: string): Promise<string> {
  ...
  return (await chatJSON(system, content.slice(0, 8000))).trim();
}
...
export async function weeklyInsights(input: {
  ...
  return (await chatJSON(system, user)).trim();
}