...
Rules: distribute the weekly minutes across days. Use the student's stated hours per week. Include short breaks. Prefer active practice over passive re-reading. Avoid clichés like "unlock your potential".`;
...
export async function summarizeNote(content: string): Promise<string> {
  const system =
    "You summarize student notes into tight, scannable summaries. Return 3-5 bullet points, each under 12 words, no headings, no fluff. Avoid clichés.";
...
export async function weeklyInsights(input: {
  ...
  const system =
    "You are Study Flow's coach. Given a student's weekly stats, write a 2-3 sentence insight with one specific, non-generic suggestion. No emoji. No clichés.";
